...
var DrawingPad = React.createClass({
  ...
  maybeDraw (e) {
    if (this.state.drawing)
      this.props.cursor.update((points) => {
        return points.push([e.clientX, e.clientY]);
      });
  },

  render () {
    var points = this.props.cursor.deref();
    return (
      <div>
        <div
          style={{
...
var App = React.createClass({
  ...
  render () {
    ...
    var textCursor = structure.cursor('text');
    var historyCount = structure.history.count();
    return (
      <div>
        <h1>{textCursor.get('title')}</h1>
        <input onChange={(e) => textCursor.update('title', () => e.target.value)}/>
        <div>
          <input
            type="range"
        ...
        <DrawingPad cursor={structure.cursor('points')}/>
      </div>
    );
  }
});
